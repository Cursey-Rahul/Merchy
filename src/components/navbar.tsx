  ...
  return (
    <div className='h-12 bg-white text-red-500 flex justify-between px-4 items-center border-b-2 border-b-red-500 md:h-20 lg:px-14 '>
      ...
      <div className='hidden md:flex items-center gap-6 text-l uppercase flex-1 justify-end lg:text-xl'>
        ...
        {!session?.user ? (
          <div>
            <Link href='/login' className='hover:underline hover:font-semibold'>Login</Link>
          </div>
        ) : (
          <div className='flex items-center gap-8'>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                <DropdownMenuItem>
                  <Link href='/orders' className='hover:underline hover:font-semibold'>My Orders</Link>
                </DropdownMenuItem>

                {/* Show Admin Page only if user is admin */}
                {session?.user?.isAdmin && (
                  <DropdownMenuItem>
                    <Link href='/admin' className='hover:underline hover:font-semibold'>Admin Page</Link>
                  </DropdownMenuItem>
                )}
