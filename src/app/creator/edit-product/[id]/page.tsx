...
const EditProductPage = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const [fetching, setFetching] = useState(false); // fixed: start false
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' && session?.user?.userType !== 'creator') router.push('/');
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      setFetching(true); // fixed: set true when fetch starts
      try {
        const baseUrl = process.env.NEXT_PUBLIC_API_URL || 'https://merchy-blond.vercel.app';
        const res = await fetch(`${baseUrl}/api/singleproduct?id=${productId}`, { cache: 'no-store' });
        ...
        // fixed: safely parse options whether string or array
        const parsedOptions = typeof data.options === 'string'
  ...
  const addOption = () => setOptions([...options, { title: '', additionalPrice: 0 }]);
      ...
      <div className='max-w-3xl mx-auto'>
        ...
        <div className='mb-6 sm:mb-8'>
          <button
            ...
            className='text-red-500 hover:underline font-semibold text-sm mb-4 flex items-center gap-1'
          >
            ← Back to Dashboard
          </button>
          <h1 className='font-bold text-2xl sm:text-3xl md:text-4xl text-gray-900 mb-2'>Edit Product</h1>
          <p className='text-gray-600 text-sm sm:text-base'>Update the details of your product</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-6 bg-white p-6 sm:p-8 rounded-lg shadow-lg'>
          ...
          <div className='border-t-2 border-gray-200 pt-6'>
            <h2 className='text-lg sm:text-xl font-bold text-gray-900 mb-4'>
              Product Options <span className='text-gray-500 font-normal text-sm'>(Optional)</span>
            </h2>
          ...
          <div className='border-t-2 border-gray-200 pt-6'>
            ...
            {imageUrl && (
              <div className='mb-4 p-4 bg-green-50 border-2 border-green-200 rounded-lg'>
                <p className='text-sm font-semibold text-green-700 mb-4'>✓ Current image</p>
                <div className='flex flex-col sm:flex-row gap-4 items-start'>
                  <div className='relative w-32 h-32 rounded-lg overflow-hidden'>
                    <Image src={imageUrl} fill alt="Product preview" className='object-cover' unoptimized />
                  </div>
                  <button
                    type="button"
